...
on:
  ...
  workflow_dispatch:
    inputs:
      mode:
        description: "Run mode: auto (today's posts + attendance) or manual (custom lookback)"
        required: false
        default: "auto"
        type: choice
        options:
          ...
          - manual
      fetch_attendance:
        description: "[manual only] Also fetch attendance? (true / false)"
        required: false
        default: "true"
      lookback_days:
        description: "[manual only] Days to look back for announcements"
        required: false
        default: "7"
