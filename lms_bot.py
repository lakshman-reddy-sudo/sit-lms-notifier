...
def is_cse2_course(course_name):
...
def main():
    ...
    mode = "MANUAL (all CSE II SEM announcements)" if MANUAL_RUN else "SCHEDULED (new announcements only)"
    print(f"[MODE] {mode}")
    ...
    # Send a header banner on manual runs
    if MANUAL_RUN:
        send_discord(
    ...
    for item in events:
        ...
        for e in event_list:
            ...
            # Skip non-CSE_II SEM courses
            if not is_cse2_course(course):
                print(f"[SKIP] Not CSE_II SEM course: {course}")
                continue
    ...
    print(f"\nDone. {sent} CSE II SEM {label} sent to Discord.")
